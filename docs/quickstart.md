...
Today that sanity check is still a manual operator step, not a dedicated CLI preflight command. Until the planned preflight work lands, use the browser troubleshooting guidance in `docs/operations-runbook.md` and validate startup with `cargo run --bin cdp_bootstrap -- --config <config-path>` (or matching `--browser-*` overrides) before spending tokens on the full proof workflow.
...
- **Browser startup sanity check.** Before `bench` or `challenge`, confirm Chromium can actually launch in your environment. A dedicated preflight command is planned but not implemented yet; for now, run `cargo run --bin cdp_bootstrap -- --config <config-path>` (or pass the same `--browser-*` flags you expect to use with `mbus`) and use `docs/operations-runbook.md` for `cdp_launch_failed` triage.
